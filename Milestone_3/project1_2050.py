...
class Course:
    ...
    def remove_waitlist(self, enrollment_record): #Milestone 2 add-on, developed by Mark Le, 
        ...
        while not self.waitlist.is_empty():
            current_record = self.waitlist.dequeue()
            if current_record != enrollment_record:
                temp_waitlist.enqueue(current_record)
...
def merge_sort(record, by): # developed by David Matos, Milestone 3, referenced class slideshow
    ...
    merge_sort(B, by)
    merge(A, B, record, by)
def merge(A, B, record, by): #helper function for merge_sort
    i = 0 #index for A
    j = 0 #index for B
    if by == "name": # used a new function lambda for better code readability 
        key = lambda r: r.student.name.lower()
    elif by == "id":
        key = lambda r: r.student.student_id
    elif by == "date":
        key = lambda r: r.enroll_date
    else: 
        raise ValueError("Choose 'name', 'id', or 'date'")
    while i < len(A) and j < len(B):
        if key(A[i]) < key(B[j]):
            ...
            record[i+j] = B[j]
            j += 1
    record[i+j:] = A[i:] + B[j:] #Add the remaining elements into the merged list   
def partition(record, i, j, by): #helper function for quick_sort
    pivot = j - 1
    j = pivot - 1
    if by == "name": # used a new function lambda for better code readability 
        key = lambda r: r.student.name.lower()
    ...
    while i < j:
        while key(record[i]) < key(record[pivot]):
            i += 1
        while j > i and key(record[j]) >= key(record[pivot]):
            j -= 1
        if i < j:
            record[i], record[j] = record[j], record[i]
    #Swap pivot and i
    if key(record[i]) > key(record[pivot]):
